...
from functions import extract_sql, try_sql_execute, fix_sql_with_llm, get_table_name_from_formatted_doc, fetch_tool_args
...
def planning_node(state: AgentState):
    ...
    args, error_message = fetch_tool_args(state, ["task_description"])
    if args is None:
        logging(f"interpret_node: {err}")
        result_payload = {"status": "error", "error_message": error_message}
    ...
    try:
        ...
        result_payload = {"status": "success", "step_answer": step_answer}
        return {
        ...
        result_payload = {"status": "error", "error_message": "プラン生成中にエラーが発生しました: {e}"}
        return {"messages": [ToolMessage(content=json.dumps(result_payload), tool_call_id=tool_call_id)]}
...
def interpret_node(state: AgentState):
    ...
    try:
        ...
        if interpretation_text:
            ...
            result_payload = {"status": "sucess", "interpretation_text": interpretation_text}
            return {"messages": [ToolMessage(content=json.dumps(result_payload), tool_call_id=tool_call_id)]}
...
def supervisor_node(state: AgentState):
    ...
    # 条件: 計画が実行中であり、かつ直前のメッセージが専門家からのToolMessageである場合
    if plan and plan_cursor is not None and len(messages) > 1 and isinstance(messages[-1], ToolMessage):
        logging.info("supervisor_node: 専門家からのToolMessageを検出。内容を評価します。")
        ...
        try:
            ...
            if isinstance(payload, dict) and payload.get("status") == "success":
                ...
                error_msg = payload.get('error_message', '不明なエラー') if isinstance(payload, dict) else str(payload)
                logging.warning(f"supervisor_node: タスク失敗または不明なステータスを検出: {error_msg}")

        except (json.JSONDecodeError, TypeError):
            # JSONパースに失敗した場合 (念のためのフォールバック)
            logging.error(f"supervisor_node: ToolMessageのcontentがJSON形式ではありません: {last_tool_message.content}")

        # タスクが成功していた場合、カーソルを更新
        if is_successful:
            new_cursor = plan_cursor + 1
            updates["plan_cursor"] = new_cursor
            logging.info(f"supervisor_node: plan_cursorを {new_cursor} に更新します。")
            
            # 計画の全ステップが完了したかチェック
            if new_cursor >= len(plan):
                logging.info("supervisor_node: 計画の全ステップが完了しました。")
                updates["messages"] = [AIMessage(content="分析計画のすべてのステップが完了しました。")]
                updates["next"] = "FINISH"
                return updates # ここで処理を終了
            
            # ローカル変数のカーソルも更新
            plan_cursor = new_cursor
        ...
        # planが存在しない場合は、これまで通り外で定義したデフォルトのChainを使います
        logging.info("supervisor_node: 計画はありません。標準プロンプトを使用します。")
        prompt = (
            "あなたはAIチームを率いる熟練のプロジェクトマネージャーです。\n"
