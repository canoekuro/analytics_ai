...
def processing_node(state: AgentState):
    ...
    - ツール実行が完了したら Final answer: で締めてください
    """
    ...
    content_payload = json.loads(agent_response["output"])
    final_df = pd.read_json(content_payload["final_df_json"])
    fig_json = content_payload["fig_json"]
    logging.info(f"processing_node: Agent response: {agent_response}")



    updated_df_history = state.get("df_history", [])
    if final_df: # new_history_itemがNoneでない（つまり有効なDataFrameが生成された）場合のみ追加
        updated_df_history = updated_df_history + [final_df]

    updated_fig = state.get("fig_history", [])
    if fig_json:
        updated_fig = updated_fig + [fig_json]
